...
% Date			: 	08-11-2017 
% Description   :   Here the signal-processor internally accumulates the 
%                   predictions for each flash event, and then sends all 
%                   the predictions in 1 big event value when requested by 
%                   sending a ('stimulus.sequence,'end') event.
%
...
while ( endTest==0 )
  ...
  [data,devents,state]=buffer_waitData(buffhost,buffport,state,'startSet','stimulus.letter','trlen_ms',trlen_ms,'exitSet',{'data' {'stimulus.feedback' 'end'}});
