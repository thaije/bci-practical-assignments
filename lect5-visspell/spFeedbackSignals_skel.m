...
while ( endTest==0 )
  ...
  [data,devents,state]=buffer_waitData(buffhost,buffport,state,'startSet','stimulus.target.letter','trlen_ms',trlen_ms,'exitSet',{'data' {'stimulus.feedback' 'end'}});
  ...
  for ei=1:numel(devents) % N.B. may be more than 1 trigger event between calls!
    ...
    elseif ( matchEvents(devents(ei),'stimulus.target.letter') ) % flash, apply the classifier
